---
name: claw-wallet
description: Install, start, and operate the Claw Wallet local sandbox; read wallet status, balances, addresses, and uid; bind wallets; sign messages and transactions; transfer assets; manage wallet security policies; and guide users through DeFi flows such as swap. Use when a user needs a local agent wallet or a policy-controlled signing flow.
version: 0.2.0
...
For non-wallet capabilities, use Hermes skill discovery instead of legacy marketplace-specific flows:

- In chat: `/skills` or `/skills search <keyword>`
- In CLI: `hermes skills list` or `hermes skills search <keyword>`
- Install optional skills with `hermes skills install <registry/category/skill>` when the workflow belongs outside this wallet skill
