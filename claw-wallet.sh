...
if [ "${1:-}" = "upgrade" ]; then
    ...
    cd "$SCRIPT_DIR"
    stop_sandbox
    CLAW_WALLET_BASE_URL="${CLAW_WALLET_BASE_URL:-https://nex-claw.vercel.app}"
    echo "Upgrading from ${CLAW_WALLET_BASE_URL}/install ..."
    export CLAW_WALLET_SKIP_INIT=1
    curl -fsSL "${CLAW_WALLET_BASE_URL}/install" | bash
    exit 0
fi
